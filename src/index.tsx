...
import { Alert, Animated, Dimensions, Platform, ScrollView, TextStyle, TouchableOpacity, View, ViewStyle } from 'react-native';
...
type TModalContentType = 'addPosition' | 'editPosition' | 'showAllPositions' | 'changeMap';

type TContentType = 'menu' | 'addPosition' | 'editPosition' | 'showAllPositions' | 'changeMap';
...
export const LinkedMap = ({
	...
	const [modalContentType, setModalContentType] = useState<TModalContentType>('showAllPositions');
	...
	const [searchedPositions, setSearchedPositions] = useState<typeof tempPositions>([]);
	...
	useEffect(() => {
		...
		if (searchTerm.length > 0) {
			_searched = _searched.filter(
				(item) =>
					item.title.toLowerCase().includes(searchTerm.toLowerCase()) || item.key.toLowerCase().includes(searchTerm.toLowerCase())
			);
		}
				...
				return (
					<View style={{ flex: 1 }}>
						...
						>
							...
							>
								<Text style={{ fontSize: 16, color: 'white' }}>Add position</Text>
							</TouchableOpacity>
									...
									{searchedPositions
										...
										.map((position, index) => {
											...
											return (
												...
												>
													<TouchableOpacity
														testID={`${testID ?? 'linkedmap'}_mapposition_${position.key}_detail`}
														onPress={() => {
													...
													<TouchableOpacity
														...
														onPress={() => {
															if (IS_WEB) {
																...
																if (result) {
																	let _mapPos = tempPositions?.filter((e) => e.key !== position.key);
																	setTempPositions(_mapPos);
																}
															} else {
																Alert.alert(
	...
	const _handleEditAction = (type: 'accept' | 'close', isDisabled?: boolean) => {
			...
			default:
				if ((contentType === 'showAllPositions' && JSON.stringify(tempPositions) !== JSON.stringify(mapPositions)) || hasChanges) {
					if (IS_WEB) {
						...
						Alert.alert('Close?', 'Do you really want to close? Any unsaved progress will be lost!', [
							{
								text: 'Cancel',
								style: 'cancel'
							},
	...
	const _handleModalAction = (type: 'accept' | 'close', isDisabled?: boolean) => {
		...
		switch (type) {
			...
			default:
				if (
					(modalContentType === 'showAllPositions' && JSON.stringify(tempPositions) !== JSON.stringify(mapPositions)) ||
					hasChanges
				) {
					if (IS_WEB) {
						const result = window.confirm('Do you really want to close? Any unsaved progress will be lost!');
	...
	const _renderModal = () => {
		const isDisabled =
			(modalContentType === 'addPosition' || modalContentType === 'editPosition') &&
			(!tempValues.title || !tempValues.target || tempValues.title === '' || tempValues.target === '');
		return (
			<View
				style={{
					flex: 1,
