...
import { Dimensions, TextInput as RNTextInput, TextInputProps, View } from 'react-native';
...
export const TextInput = (props: ITextInputComponent) => {
	const { bold, center, children, italic, label, largerText, onlyNumbers, style } = props;
	...
	return (
		...
		>
			{label && Dimensions.get('window').width > 500 && (
				<Text bold numberOfLines={1} style={{ fontSize: largerText ? 20 : 15, marginRight: 8 }}>
					{label}
				</Text>
			)}
			<RNTextInput
